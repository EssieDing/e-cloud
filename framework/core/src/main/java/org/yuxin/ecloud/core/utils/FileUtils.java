package org.yuxin.ecloud.core.utils;
...
public class FileUtils {
    ...
    public static String getFileSuffix(String filename) {
        if (StringUtils.isBlank(filename) || filename.lastIndexOf(ECloudConstants.POINT_STR) == ECloudConstants.MINUS_ONE_INT) {
            return ECloudConstants.EMPTY_STR;
        }
        return filename.substring(filename.lastIndexOf(ECloudConstants.POINT_STR)).toLowerCase();
    }
    ...
    public static String getFileExtName(String filename) {
        ...
        return filename.substring(filename.lastIndexOf(ECloudConstants.POINT_STR) + ECloudConstants.ONE_INT).toLowerCase();
    }
    ...
    public static String generateStoreFileChunkRealPath(String basePath, String identifier, Integer chunkNumber) {
        return new StringBuffer(basePath)
                ...
                .append(File.separator)
                .append(UUIDUtil.getUUID())
                .append(ECloudConstants.COMMON_SEPARATOR)
                .append(chunkNumber)
                .toString();
    }
    ...
    public static void writeFile2OutputStream(FileInputStream fileInputStream, OutputStream outputStream, long length) throws IOException {
        ...
        fileChannel.transferTo(ECloudConstants.ZERO_LONG, length, writableByteChannel);
        outputStream.flush();
    ...
    public static void writeStream2StreamNormal(InputStream inputStream, OutputStream outputStream) throws IOException {
        ...
        int len;
        while ((len = inputStream.read(buffer)) != ECloudConstants.MINUS_ONE_INT) {
            outputStream.write(buffer, ECloudConstants.ZERO_INT, len);
        }
        outputStream.flush();
        inputStream.close();
        outputStream.close();
