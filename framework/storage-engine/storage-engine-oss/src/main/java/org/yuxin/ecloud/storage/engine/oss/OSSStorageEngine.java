package org.yuxin.ecloud.storage.engine.oss;
...
import org.yuxin.ecloud.storage.engine.core.AbstractStorageEngine;
import org.yuxin.ecloud.storage.engine.core.context.*;
...
public class OSSStorageEngine extends AbstractStorageEngine {
    ...
    protected void doStoreChunk(StoreFileChunkContext context) throws IOException {
        ...
        if (context.getTotalChunks() > TEN_THOUSAND_INT) {
            throw new ECloudFrameworkException("分片数超过了限制，分片数不得大于： " + TEN_THOUSAND_INT);
        }
        ...
        if (Objects.isNull(result)) {
            throw new ECloudFrameworkException("文件分片上传失败");
        }
    ...
    protected void doMergeFile(MergeFileContext context) throws IOException {
        ...
        if (Objects.isNull(entity)) {
            throw new ECloudFrameworkException("文件分片合并失败，文件的唯一标识为：" + context.getIdentifier());
        }
    ...
    protected void doReadFile(ReadFileContext context) throws IOException {
        ...
        if (Objects.isNull(ossObject)) {
            throw new ECloudFrameworkException("文件读取失败，文件的名称为：" + context.getRealPath());
        }
    ...
    private String getFilePath(String fileSuffix) {
        return new StringBuffer()
                ...
                .append(ECloudConstants.SLASH_STR)
    ...
    private String assembleUrl(String baseUrl, JSONObject params) {
        ...
        urlStringBuffer.append(ECloudConstants.QUESTION_MARK_STR);
        List<String> paramsList = Lists.newArrayList();
        StringBuffer urlParamsStringBuffer = new StringBuffer();
        params.entrySet().forEach(entry -> {
            urlParamsStringBuffer.setLength(ECloudConstants.ZERO_INT);
            urlParamsStringBuffer.append(entry.getKey());
            urlParamsStringBuffer.append(ECloudConstants.EQUALS_MARK_STR);
            urlParamsStringBuffer.append(entry.getValue());
            paramsList.add(urlParamsStringBuffer.toString());
        });
        return urlStringBuffer.append(Joiner.on(ECloudConstants.AND_MARK_STR).join(paramsList)).toString();
    }
    ...
    private String getBaseUrl(String url) {
        if (StringUtils.isBlank(url)) {
            return ECloudConstants.EMPTY_STR;
        }
        if (checkHaveParams(url)) {
            return url.split(getSplitMark(ECloudConstants.QUESTION_MARK_STR))[0];
        }
        return url;
    }
    ...
    private String getSplitMark(String mark) {
        return new StringBuffer(ECloudConstants.LEFT_BRACKET_STR)
                .append(mark)
                .append(ECloudConstants.RIGHT_BRACKET_STR)
                .toString();
    }
    ...
    private JSONObject analysisUrlParams(String url) {
        ...
        String paramsPart = url.split(getSplitMark(ECloudConstants.QUESTION_MARK_STR))[1];
        if (StringUtils.isNotBlank(paramsPart)) {
            List<String> paramPairList = Splitter.on(ECloudConstants.AND_MARK_STR).splitToList(paramsPart);
            paramPairList.stream().forEach(paramPair -> {
                String[] paramArr = paramPair.split(getSplitMark(ECloudConstants.EQUALS_MARK_STR));
                if (paramArr != null && paramArr.length == ECloudConstants.TWO_INT) {
    ...
    private boolean checkHaveParams(String url) {
        return StringUtils.isNotBlank(url) && url.indexOf(ECloudConstants.QUESTION_MARK_STR) != ECloudConstants.MINUS_ONE_INT;
    }
    ...
    private ChunkUploadEntity initChunkUpload(String filename, String cacheKey) {
        ...
        if (Objects.isNull(result)) {
            throw new ECloudFrameworkException("文件分片上传初始化失败");
        }
