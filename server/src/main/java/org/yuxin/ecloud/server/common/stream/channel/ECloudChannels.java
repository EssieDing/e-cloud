package org.yuxin.ecloud.server.common.stream.channel;
 ...
 * 事件通道定义类
 */
public interface ECloudChannels {
    ...
    @Input(ECloudChannels.ERROR_LOG_INPUT)
    SubscribableChannel errorLogInput();

    @Output(ECloudChannels.ERROR_LOG_OUTPUT)
    MessageChannel errorLogOutput();

    @Input(ECloudChannels.DELETE_FILE_INPUT)
    SubscribableChannel deleteFileInput();

    @Output(ECloudChannels.DELETE_FILE_OUTPUT)
    MessageChannel deleteFileOutput();

    @Input(ECloudChannels.FILE_RESTORE_INPUT)
    SubscribableChannel fileRestoreInput();

    @Output(ECloudChannels.FILE_RESTORE_OUTPUT)
    MessageChannel fileRestoreOutput();

    @Input(ECloudChannels.PHYSICAL_DELETE_FILE_INPUT)
    SubscribableChannel physicalDeleteFileInput();

    @Output(ECloudChannels.PHYSICAL_DELETE_FILE_OUTPUT)
    MessageChannel physicalDeleteFileOutput();

    @Input(ECloudChannels.USER_SEARCH_INPUT)
    SubscribableChannel userSearchInput();

    @Output(ECloudChannels.USER_SEARCH_OUTPUT)
