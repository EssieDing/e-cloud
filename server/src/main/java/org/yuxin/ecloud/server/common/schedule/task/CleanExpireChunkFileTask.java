package org.yuxin.ecloud.server.common.schedule.task;

import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import org.yuxin.ecloud.core.constants.ECloudConstants;
import org.yuxin.ecloud.schedule.ScheduleTask;
...
public class CleanExpireChunkFileTask implements ScheduleTask {
    ...
    public void run() {
        ...
        List<ECloudFileChunk> expireFileChunkRecords;
     ...
     * @return
     */
    private List<ECloudFileChunk> scrollQueryExpireFileChunkRecords(Long scrollPointer) {
     ...
     */
    private void deleteRealChunkFiles(List<ECloudFileChunk> expireFileChunkRecords) {
        DeleteFileContext deleteFileContext = new DeleteFileContext();
        List<String> realPaths = expireFileChunkRecords.stream().map(ECloudFileChunk::getRealPath).collect(Collectors.toList());
    ...
    private void saveErrorLog(List<String> realPaths) {
        ErrorLogEvent event = new ErrorLogEvent("文件物理删除失败，请手动执行文件删除！文件路径为：" + JSON.toJSONString(realPaths), ECloudConstants.ZERO_LONG);
        producer.sendMessage(ECloudChannels.ERROR_LOG_OUTPUT, event);
    }
     ...
     * @return
     */
    private List<Long> deleteChunkFileRecords(List<ECloudFileChunk> expireFileChunkRecords) {
        List<Long> idList = expireFileChunkRecords.stream().map(ECloudFileChunk::getId).collect(Collectors.toList());
