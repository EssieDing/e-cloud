...
import org.yuxin.ecloud.server.common.stream.event.file.DeleteFileEvent;
import org.yuxin.ecloud.server.common.stream.event.search.UserSearchEvent;
import org.yuxin.ecloud.server.common.utils.HttpUtil;
...
import org.yuxin.ecloud.server.modules.file.enums.FileTypeEnum;
import org.yuxin.ecloud.server.modules.file.enums.FolderFlagEnum;
import org.yuxin.ecloud.server.modules.file.mapper.ECloudUserFileMapper;
...
import org.yuxin.ecloud.storage.engine.core.context.ReadFileContext;
...
/**
 * @author yuxin
 * @description 针对表【e_cloud_user_file(用户文件信息表)】的数据库操作Service实现
 * @createDate 2023-05-20 18:36:41
 */
@Service(value = "userFileService")
public class UserFileServiceImpl extends ServiceImpl<ECloudUserFileMapper, ECloudUserFile> implements IUserFileService {
     ...
     */
    @Override
    public ECloudUserFile getUserRootFile(Long userId) {
     ...
     */
    @Override
    public List<ECloudUserFileVO> getFileList(QueryFileListContext context) {
    ...
    public boolean secUpload(SecUploadFileContext context) {
        List<ECloudFile> fileList = getFileListByUserIdAndIdentifier(context.getUserId(), context.getIdentifier());
        if (CollectionUtils.isNotEmpty(fileList)) {
            ECloudFile record = fileList.get(ECloudConstants.ZERO_INT);
    ...
    public void download(FileDownloadContext context) {
        ECloudUserFile record = getById(context.getFileId());
        checkOperatePermission(record, context.getUserId());
        if (checkIsFolder(record)) {
            throw new ECloudBusinessException("文件夹暂不支持下载");
        }
    ...
    public void downloadWithoutCheckUser(FileDownloadContext context) {
        ...
        if (Objects.isNull(record)) {
            throw new ECloudBusinessException("当前文件记录不存在");
        }
        if (checkIsFolder(record)) {
            throw new ECloudBusinessException("文件夹暂不支持下载");
        }
    ...
    public List<FolderTreeNodeVO> getFolderTree(QueryFolderTreeContext context) {
        List<ECloudUserFile> folderRecords = queryFolderRecords(context.getUserId());
     ...
     */
    @Override
    public List<ECloudUserFile> findAllFileRecords(List<ECloudUserFile> records) {
        List<ECloudUserFile> result = Lists.newArrayList(records);
     ...
     */
    @Override
    public List<ECloudUserFile> findAllFileRecordsByFileIdList(List<Long> fileIdList) {
        if (CollectionUtils.isEmpty(fileIdList)) {
            return Lists.newArrayList();
        }
        List<ECloudUserFile> records = listByIds(fileIdList);
     ...
     */
    @Override
    public List<ECloudUserFileVO> transferVOList(List<ECloudUserFile> records) {
        if (CollectionUtils.isEmpty(records)) {
            return Lists.newArrayList();
        }
        return records.stream().map(fileConverter::eCloudUserFile2ECloudUserFileVO).collect(Collectors.toList());
    }
     ...
     * @param record
     */
    private void doFindAllChildRecords(List<ECloudUserFile> result, ECloudUserFile record) {
        if (Objects.isNull(record)) {
            return;
        }
        if (!checkIsFolder(record)) {
        ...
        List<ECloudUserFile> childRecords = findChildRecordsIgnoreDelFlag(record.getFileId());
     ...
     * @return
     */
    private List<ECloudUserFile> findChildRecordsIgnoreDelFlag(Long fileId) {
        QueryWrapper queryWrapper = Wrappers.query();
        queryWrapper.eq("parent_id", fileId);
        List<ECloudUserFile> childRecords = list(queryWrapper);
        return childRecords;
    }
    ...
    private void afterSearch(FileSearchContext context) {
        ...
        producer.sendMessage(ECloudChannels.USER_SEARCH_OUTPUT, event);
    }
    ...
    private void fillParentFilename(List<FileSearchResultVO> result) {
        ...
        List<ECloudUserFile> parentRecords = listByIds(parentIdList);
        Map<Long, String> fileId2filenameMap = parentRecords.stream().collect(Collectors.toMap(ECloudUserFile::getFileId, ECloudUserFile::getFilename));
    ...
    private void doCopy(CopyFileContext context) {
        List<ECloudUserFile> prepareRecords = context.getPrepareRecords();
        if (CollectionUtils.isNotEmpty(prepareRecords)) {
            List<ECloudUserFile> allRecords = Lists.newArrayList();
            ...
            if (!saveBatch(allRecords)) {
                throw new ECloudBusinessException("文件复制失败");
            }
        }
    }
     ...
     * @param userId
     */
    private void assembleCopyChildRecord(List<ECloudUserFile> allRecords, ECloudUserFile record, Long targetParentId, Long userId) {
        ...
        if (checkIsFolder(record)) {
            List<ECloudUserFile> childRecords = findChildRecords(oldFileId);
            if (CollectionUtils.isEmpty(childRecords)) {
                return;
            }
     ...
     * @return
     */
    private List<ECloudUserFile> findChildRecords(Long parentId) {
        QueryWrapper queryWrapper = Wrappers.query();
    ...
    private void checkCopyCondition(CopyFileContext context) {
        ...
        if (!checkIsFolder(getById(targetParentId))) {
            throw new ECloudBusinessException("目标文件不是一个文件夹");
        }
        List<Long> fileIdList = context.getFileIdList();
        List<ECloudUserFile> prepareRecords = listByIds(fileIdList);
        context.setPrepareRecords(prepareRecords);
        if (checkIsChildFolder(prepareRecords, targetParentId, context.getUserId())) {
            throw new ECloudBusinessException("目标文件夹ID不能是选中文件列表的文件夹ID或其子文件夹ID");
        }
    }
    ...
    private void doTransfer(TransferFileContext context) {
        ...
        if (!updateBatchById(prepareRecords)) {
            throw new ECloudBusinessException("文件转移失败");
        }
    }
     ...
     * @return
     */
    private boolean checkIsChildFolder(List<ECloudUserFile> prepareRecords, Long targetParentId, Long userId) {
        ...
        List<ECloudUserFile> folderRecords = queryFolderRecords(userId);
        Map<Long, List<ECloudUserFile>> folderRecordMap = folderRecords.stream().collect(Collectors.groupingBy(ECloudUserFile::getParentId));
        List<ECloudUserFile> unavailableFolderRecords = Lists.newArrayList();
        ...
        List<Long> unavailableFolderRecordIds = unavailableFolderRecords.stream().map(ECloudUserFile::getFileId).collect(Collectors.toList());
     ...
     * @param record
     */
    private void findAllChildFolderRecords(List<ECloudUserFile> unavailableFolderRecords, Map<Long, List<ECloudUserFile>> folderRecordMap, ECloudUserFile record) {
        if (Objects.isNull(record)) {
            return;
        }
        List<ECloudUserFile> childFolderRecords = folderRecordMap.get(record.getFileId());
     ...
     * @return
     */
    private List<FolderTreeNodeVO> assembleFolderTreeNodeVOList(List<ECloudUserFile> folderRecords) {
        if (CollectionUtils.isEmpty(folderRecords)) {
            return Lists.newArrayList();
        }
        List<FolderTreeNodeVO> mappedFolderTreeNodeVOList = folderRecords.stream().map(fileConverter::eCloudUserFile2FolderTreeNodeVO).collect(Collectors.toList());
     ...
     * @return
     */
    private List<ECloudUserFile> queryFolderRecords(Long userId) {
        QueryWrapper queryWrapper = Wrappers.query();
        queryWrapper.eq("user_id", userId);
        queryWrapper.eq("folder_flag", FolderFlagEnum.YES.getCode());
        queryWrapper.eq("del_flag", DelFlagEnum.NO.getCode());
     ...
     * @param response
     */
    private void doPreview(ECloudUserFile record, HttpServletResponse response) {
        ECloudFile realFileRecord = iFileService.getById(record.getRealFileId());
        if (Objects.isNull(realFileRecord)) {
            throw new ECloudBusinessException("当前的文件记录不存在");
        }
     ...
     * @param response
     */
    private void doDownload(ECloudUserFile record, HttpServletResponse response) {
        ECloudFile realFileRecord = iFileService.getById(record.getRealFileId());
        if (Objects.isNull(realFileRecord)) {
            throw new ECloudBusinessException("当前的文件记录不存在");
        }
    ...
    private void realFile2OutputStream(String realPath, HttpServletResponse response) {
        try {
            ...
            e.printStackTrace();
            throw new ECloudBusinessException("文件下载失败");
        }
    }
     ...
     */
    private void addDownloadAttribute(HttpServletResponse response, ECloudUserFile record, ECloudFile realFileRecord) {
        try {
     ...
     * @param userId
     */
    private void checkOperatePermission(ECloudUserFile record, Long userId) {
        if (Objects.isNull(record)) {
            throw new ECloudBusinessException("当前文件记录不存在");
        }
        if (!record.getUserId().equals(userId)) {
            throw new ECloudBusinessException("您没有该文件的操作权限");
        }
    }
     ...
     * @return
     */
    private boolean checkIsFolder(ECloudUserFile record) {
        if (Objects.isNull(record)) {
            throw new ECloudBusinessException("当前文件记录不存在");
        }
     ...
     * @return
     */
    private List<ECloudFile> getFileListByUserIdAndIdentifier(Long userId, String identifier) {
    ...
    private void afterFileDelete(DeleteFileContext context) {
        ...
        producer.sendMessage(ECloudChannels.DELETE_FILE_OUTPUT, deleteFileEvent);
    }
    ...
    private void checkFileDeleteCondition(DeleteFileContext context) {
        ...
        List<ECloudUserFile> eCloudUserFiles = listByIds(fileIdList);
        if (eCloudUserFiles.size() != fileIdList.size()) {
            throw new ECloudBusinessException("存在不合法的文件记录");
        }

        Set<Long> fileIdSet = eCloudUserFiles.stream().map(ECloudUserFile::getFileId).collect(Collectors.toSet());
        ...
        Set<Long> userIdSet = eCloudUserFiles.stream().map(ECloudUserFile::getUserId).collect(Collectors.toSet());
        if (userIdSet.size() != 1) {
            throw new ECloudBusinessException("存在不合法的文件记录");
        }
        ...
        if (!Objects.equals(dbUserId, context.getUserId())) {
            throw new ECloudBusinessException("当前登录用户没有删除该文件的权限");
        }
    }
    ...
    private Long saveUserFile(Long parentId,
                              ...
                              Long userId,
                              String fileSizeDesc) {
        ECloudUserFile entity = assembleECloudUserFile(parentId, userId, filename, folderFlagEnum, fileType, realFileId, fileSizeDesc);
        if (!save((entity))) {
            throw new ECloudBusinessException("保存文件信息失败");
        }
     ...
     * @return
     */
    private ECloudUserFile assembleECloudUserFile(Long parentId, Long userId, String filename, FolderFlagEnum folderFlagEnum, Integer fileType, Long realFileId, String fileSizeDesc) {
        ECloudUserFile entity = new ECloudUserFile();
     ...
     * @param entity
     */
    private void handleDuplicateFilename(ECloudUserFile entity) {
        String filename = entity.getFilename(),
                newFilenameWithoutSuffix,
                newFilenameSuffix;
        int newFilenamePointPosition = filename.lastIndexOf(ECloudConstants.POINT_STR);
        if (newFilenamePointPosition == ECloudConstants.MINUS_ONE_INT) {
            newFilenameWithoutSuffix = filename;
            newFilenameSuffix = StringUtils.EMPTY;
        } else {
            newFilenameWithoutSuffix = filename.substring(ECloudConstants.ZERO_INT, newFilenamePointPosition);
            newFilenameSuffix = filename.replace(newFilenameWithoutSuffix, StringUtils.EMPTY);
        }

        List<ECloudUserFile> existRecords = getDuplicateFilename(entity, newFilenameWithoutSuffix);

        if (CollectionUtils.isEmpty(existRecords)) {
            return;
        }

        List<String> existFilenames = existRecords.stream().map(ECloudUserFile::getFilename).collect(Collectors.toList());
     ...
     * @return
     */
    private List<ECloudUserFile> getDuplicateFilename(ECloudUserFile entity, String newFilenameWithoutSuffix) {
        QueryWrapper queryWrapper = new QueryWrapper();
    ...
    private void doUpdateFilename(UpdateFilenameContext context) {
        ECloudUserFile entity = context.getEntity();
        entity.setFilename(context.getNewFilename());
        entity.setUpdateUser(context.getUserId());
        entity.setUpdateTime(new Date());

        if (!updateById(entity)) {
            throw new ECloudBusinessException("文件重命名失败");
        }
    }
    ...
    private void checkUpdateFilenameCondition(UpdateFilenameContext context) {
        ...
        Long fileId = context.getFileId();
        ECloudUserFile entity = getById(fileId);

        if (Objects.isNull(entity)) {
            throw new ECloudBusinessException("该文件ID无效");
        }

        if (!Objects.equals(entity.getUserId(), context.getUserId())) {
            throw new ECloudBusinessException("当前登录用户没有修改该文件名称的权限");
        }

        if (Objects.equals(entity.getFilename(), context.getNewFilename())) {
            throw new ECloudBusinessException("请换一个新的文件名称来修改");
        }
        ...
        if (count > 0) {
            throw new ECloudBusinessException("该文件名称已被占用");
        }
