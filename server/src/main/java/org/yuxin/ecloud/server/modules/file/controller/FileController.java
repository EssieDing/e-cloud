package org.yuxin.ecloud.server.modules.file.controller;
...
public class FileController {
    ...
    @GetMapping("files")
    public R<List<ECloudUserFileVO>> list(@NotBlank(message = "父文件夹ID不能为空") @RequestParam(value = "parentId", required = false) String parentId,
        ...
        if (!Objects.equals(FileConstants.ALL_FILE_TYPE, fileTypes)) {
            fileTypeArray = Splitter.on(ECloudConstants.COMMON_SEPARATOR)
    ...
    public R<List<FileSearchResultVO>> search(@Validated FileSearchPO fileSearchPO) {
        ...
        if (StringUtils.isNotBlank(fileTypes) && !Objects.equals(FileConstants.ALL_FILE_TYPE, fileTypes)) {
            List<Integer> fileTypeArray = Splitter.on(ECloudConstants.COMMON_SEPARATOR).splitToList(fileTypes).stream().map(Integer::valueOf).collect(Collectors.toList());
            context.setFileTypeArray(fileTypeArray);
        }
