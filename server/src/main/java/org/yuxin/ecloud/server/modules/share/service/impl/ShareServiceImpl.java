...
import org.yuxin.ecloud.bloom.filter.core.BloomFilterManager;
...
import org.yuxin.ecloud.core.utils.UUIDUtil;
import org.yuxin.ecloud.server.common.cache.ManualCacheService;
...
import org.yuxin.ecloud.server.modules.file.context.CopyFileContext;
import org.yuxin.ecloud.server.modules.file.context.FileDownloadContext;
...
import org.yuxin.ecloud.server.modules.share.constants.ShareConstants;
import org.yuxin.ecloud.server.modules.share.context.*;
import org.yuxin.ecloud.server.modules.share.entity.ECloudShare;
import org.yuxin.ecloud.server.modules.share.enums.ShareDayTypeEnum;
import org.yuxin.ecloud.server.modules.share.enums.ShareStatusEnum;
...
import org.yuxin.ecloud.server.modules.share.vo.*;
...
/**
 * @author yuxin
 * @description 针对表【e_cloud_share(用户分享表)】的数据库操作Service实现
 * @createDate 2023-05-20 18:38:38
 */
@Service
@Slf4j
public class ShareServiceImpl extends ServiceImpl<ECloudShareMapper, ECloudShare> implements IShareService {
    ...
    private ManualCacheService<ECloudShare> cacheService;
     ...
     * @return
     */
    @Transactional(rollbackFor = ECloudBusinessException.class)
    @Override
    public ECloudShareUrlVO create(CreateShareUrlContext context) {
        saveShare(context);
        saveShareFiles(context);
        ECloudShareUrlVO vo = assembleShareVO(context);
     ...
     */
    @Override
    public List<ECloudShareUrlListVO> getShares(QueryShareListContext context) {
    ...
    public String checkShareCode(CheckShareCodeContext context) {
        ECloudShare record = checkShareStatus(context.getShareId());
     ...
     */
    @Override
    public List<ECloudUserFileVO> fileList(QueryChildFileListContext context) {
        ECloudShare record = checkShareStatus(context.getShareId());
        context.setRecord(record);
        List<ECloudUserFileVO> allUserFileRecords = checkFileIdIsOnShareStatusAndGetAllShareUserFiles(context.getShareId(), Lists.newArrayList(context.getParentId()));
        Map<Long, List<ECloudUserFileVO>> parentIdFileListMap = allUserFileRecords.stream().collect(Collectors.groupingBy(ECloudUserFileVO::getParentId));
        List<ECloudUserFileVO> eCloudUserFileVOS = parentIdFileListMap.get(context.getParentId());
        if (CollectionUtils.isEmpty(eCloudUserFileVOS)) {
            return Lists.newArrayList();
        }
        return eCloudUserFileVOS;
    }
...
//        return super.removeByIds(idList);
     ...
     */
    @Override
    public boolean updateById(ECloudShare entity) {
...
//        return super.updateById(entity);
     ...
     */
    @Override
    public boolean updateBatchById(Collection<ECloudShare> entityList) {
        if (CollectionUtils.isEmpty(entityList)) {
            return true;
        }
        Map<Long, ECloudShare> entityMap = entityList.stream().collect(Collectors.toMap(ECloudShare::getShareId, e -> e));
...
//        return super.updateBatchById(entityList);
     ...
     */
    @Override
    public ECloudShare getById(Serializable id) {
...
//        return super.getById(id);
     ...
     */
    @Override
    public List<ECloudShare> listByIds(Collection<? extends Serializable> idList) {
...
//        return super.listByIds(idList);
    ...
    private void refreshOneShareStatus(Long shareId) {
        ECloudShare record = getById(shareId);
     ...
     * @param shareStatus
     */
    private void doChangeShareStatus(ECloudShare record, ShareStatusEnum shareStatus) {
        record.setShareStatus(shareStatus.getCode());
        if (!updateById(record)) {
            producer.sendMessage(ECloudChannels.ERROR_LOG_OUTPUT, new ErrorLogEvent("更新分享状态失败，请手动更改状态，分享ID为：" + record.getShareId() + ", 分享" +
                    "状态改为：" + shareStatus.getCode(), ECloudConstants.ZERO_LONG));
        }
    }
    ...
    private boolean checkUpFileAvailable(Long fileId) {
        ECloudUserFile record = iUserFileService.getById(fileId);
        if (Objects.isNull(record)) {
            return false;
        }
     ...
     * @return
     */
    private List<ECloudUserFileVO> checkFileIdIsOnShareStatusAndGetAllShareUserFiles(Long shareId, List<Long> fileIdList) {
        List<Long> shareFileIdList = getShareFileIdList(shareId);
        if (CollectionUtils.isEmpty(shareFileIdList)) {
            return Lists.newArrayList();
        }
        List<ECloudUserFile> allFileRecords = iUserFileService.findAllFileRecordsByFileIdList(shareFileIdList);
        ...
        List<Long> allFileIdList = allFileRecords.stream().map(ECloudUserFile::getFileId).collect(Collectors.toList());

        if (allFileIdList.containsAll(fileIdList)) {
            return iUserFileService.transferVOList(allFileRecords);
        }

        throw new ECloudBusinessException(ResponseCode.SHARE_FILE_MISS);
    }
    ...
    private void assembleShareSimpleUserInfo(QueryShareSimpleDetailContext context) {
        ECloudUser record = iUserService.getById(context.getRecord().getCreateUser());
        if (Objects.isNull(record)) {
    ...
    private void assembleMainShareSimpleInfo(QueryShareSimpleDetailContext context) {
        ECloudShare record = context.getRecord();
    ...
    private String encryptUsername(String username) {
        ...
        stringBuffer.replace(ECloudConstants.TWO_INT, username.length() - ECloudConstants.TWO_INT, ECloudConstants.COMMON_ENCRYPT_STR);
    ...
    private void assembleShareFilesInfo(QueryShareDetailContext context) {
        ...
        List<ECloudUserFileVO> eCloudUserFileVOList = iUserFileService.getFileList(queryFileListContext);
        context.getVo().setECloudUserFileVOList(eCloudUserFileVOList);
    }
    ...
    private void doCheckShareCode(CheckShareCodeContext context) {
        ...
        if (!Objects.equals(context.getShareCode(), record.getShareCode())) {
            throw new ECloudBusinessException("分享码错误");
        }
    }
     ...
     * @return
     */
    private ECloudShare checkShareStatus(Long shareId) {
        ECloudShare record = getById(shareId);

        if (Objects.isNull(record)) {
            throw new ECloudBusinessException(ResponseCode.SHARE_CANCELLED);
        }
        ...
        if (record.getShareEndTime().before(new Date())) {
            throw new ECloudBusinessException(ResponseCode.SHARE_EXPIRE);
        }
    ...
    private void doCancelShareFiles(CancelShareContext context) {
        ...
        if (!iShareFileService.remove(queryWrapper)) {
            throw new ECloudBusinessException("取消分享失败");
        }
    }
    ...
    private void checkUserCancelSharePermission(CancelShareContext context) {
        ...
        List<ECloudShare> records = listByIds(shareIdList);
        if (CollectionUtils.isEmpty(records)) {
            throw new ECloudBusinessException("您无权限操作取消分享的动作");
        }
        for (ECloudShare record : records) {
     ...
     * @return
     */
    private ECloudShareUrlVO assembleShareVO(CreateShareUrlContext context) {
        ECloudShare record = context.getRecord();
        ECloudShareUrlVO vo = new ECloudShareUrlVO();
        vo.setShareId(record.getShareId());
        vo.setShareName(record.getShareName());
    ...
    private void saveShare(CreateShareUrlContext context) {
        ECloudShare record = new ECloudShare();
        ...
        if (Objects.equals(ECloudConstants.MINUS_ONE_INT, shareDay)) {
            throw new ECloudBusinessException("分享天数非法");
        }
        ...
        if (!save(record)) {
            throw new ECloudBusinessException("保存分享信息失败");
        }
    ...
    private String createShareUrl(Long shareId) {
        if (Objects.isNull(shareId)) {
            throw new ECloudBusinessException("分享的ID不能为空");
        }
        String sharePrefix = config.getSharePrefix();
        if (!sharePrefix.endsWith(ECloudConstants.SLASH_STR)) {
            sharePrefix += ECloudConstants.SLASH_STR;
        }
     ...
     * @param vo
     */
    private void afterCreate(CreateShareUrlContext context, ECloudShareUrlVO vo) {
