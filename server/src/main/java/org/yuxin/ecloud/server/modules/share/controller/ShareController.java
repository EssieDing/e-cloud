package org.yuxin.ecloud.server.modules.share.controller;
...
import org.yuxin.ecloud.server.common.annotation.NeedShareCode;
import org.yuxin.ecloud.server.common.utils.ShareIdUtil;
...
import org.yuxin.ecloud.server.modules.share.converter.ShareConverter;
import org.yuxin.ecloud.server.modules.share.po.CancelSharePO;
import org.yuxin.ecloud.server.modules.share.po.CheckShareCodePO;
import org.yuxin.ecloud.server.modules.share.po.CreateShareUrlPO;
import org.yuxin.ecloud.server.modules.share.po.ShareSavePO;
...
public class ShareController {
    ...
    @PostMapping("share")
    public R<ECloudShareUrlVO> create(@Validated @RequestBody CreateShareUrlPO createShareUrlPO) {
        ...
        List<Long> shareFileIdList = Splitter.on(ECloudConstants.COMMON_SEPARATOR).splitToList(shareFileIds).stream().map(IdUtil::decrypt).collect(Collectors.toList());

        context.setShareFileIdList(shareFileIdList);

        ECloudShareUrlVO vo = iShareService.create(context);
        return R.data(vo);
    }
    ...
    @GetMapping("shares")
    public R<List<ECloudShareUrlListVO>> getShares() {
        QueryShareListContext context = new QueryShareListContext();
        context.setUserId(UserIdUtil.get());
        List<ECloudShareUrlListVO> result = iShareService.getShares(context);
    ...
    public R cancelShare(@Validated @RequestBody CancelSharePO cancelSharePO) {
        ...
        List<Long> shareIdList = Splitter.on(ECloudConstants.COMMON_SEPARATOR).splitToList(shareIds).stream().map(IdUtil::decrypt).collect(Collectors.toList());
    ...
    @NeedShareCode
    @LoginIgnore
    public R<List<ECloudUserFileVO>> fileList(@NotBlank(message = "文件的父ID不能为空") @RequestParam(value = "parentId", required = false) String parentId) {
        QueryChildFileListContext context = new QueryChildFileListContext();
        context.setShareId(ShareIdUtil.get());
        context.setParentId(IdUtil.decrypt(parentId));
        List<ECloudUserFileVO> result = iShareService.fileList(context);
        return R.data(result);
    }
